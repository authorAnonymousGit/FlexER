...
def predict(input_path, output_path, config, model, intent=1,
            batch_size=1024,
            summarizer=None,
            lm='distilbert',
            max_len=256,
    ...
    def process_batch(rows, pairs, writer):
        try:
            predictions, logits, poolers = classify(pairs, config, model, lm=lm, max_len=max_len)
        except:
...
def load_model(task, path, lm, use_gpu, intent, fp16=True):
    ...
    for intent in range(hp.intents_num):
        for file_type in file_types:
            ...
            config, model = load_model(task, hp.checkpoint_path,
                                       hp.lm, hp.use_gpu, intent, hp.fp16)
            ...
            # run prediction
            predict(input_path, output_path, config, model, intent,

...
def create_dataloaders_inference(files_path, task, intent, intents_num, emb_type, batch_size):
...
def epoch_train(model, train_dataloader, criterion, optimizer, device, intent, intents_num, scheduler, batch_size):
    model.train()
...
def epoch_test(model, rel_dataloader, criterion, device, intent, intents_num):
    model.eval()
    correct_preds = 0.0
    total_loss = 0.0
    predictions_list = []
...
def run_graph_classification(files_path, task, intent,
    ...
    device = find_device()
    features_num = get_features_num(emb_type)
